...
(function() {
    ...
    // --- OPTIONAL OVERRIDE: user-chosen instance from the settings panel ---
    // If a value was saved on an Invidious instance page (key 'yt.instance'),
    // validate and normalise it the same way as the in-file default above:
    // force https, drop any path/query. A blank value falls back to the
    // in-file default.
    try {
        var savedInstance = storeGet('yt.instance', '');
        if (savedInstance) {
            var parsedSaved = new URL(savedInstance);
            if (parsedSaved.protocol === 'https:' || parsedSaved.protocol === 'http:') {
                invidious = 'https://' + parsedSaved.host; // force https, drop any path/query
                invidiousHost = parsedSaved.hostname;
            }
        }
    } catch (e) { /* invalid saved value — keep the in-file default */ }
    ...
    // The master switch lives in storage; combined with the one-time bypass
    // above it decides whether the main YouTube redirect path may run.
    var redirectActive = enabled && !bypass;

    // --- GUARD: Don't run YouTube logic if we're already on the Invidious instance ---
    // Prevents redirect loops if the script ever runs on the Invidious page itself
    if (window.location.hostname === invidiousHost) {
        // On the front-end instance itself: no redirect — instead show the
        // settings panel so the user can manage the instance override and
        // the enable/disable toggle.
        whenHead(function() {
            buildSettingsPanel({
                title: 'YouTube → Invidious',
                fields: [
                    { key: 'instance', label: 'Invidious instance:', placeholder: 'https://inv.example.com', value: storeGet('yt.instance', invidious) }
                ],
                onSave: function(v) {
                    storeSet('yt.instance', v.instance);
                }
            });
        });
        return;
    ...
    // Register a menu command for quick access. The settings panel itself is
    // only injected on the Invidious instance (see the loop guard above).
    registerMenu('YouTube → Invidious settings', function() { /* panel lives on the instance page */ });
    ...
    // Skip all YouTube redirect logic if we're not on a YouTube domain, or if
    // the redirect is disabled (or a one-time "#noredirect" bypass was given).
    // In that case we neither hide the body nor redirect — the page loads
    // normally and we fall through to the DDG section below (which no-ops off
    // DuckDuckGo). The body-hiding <style> below is therefore only applied on
    // an active (redirectActive), top-level, YouTube-domain path.
    if (!isYouTubeDomain || !redirectActive) {
